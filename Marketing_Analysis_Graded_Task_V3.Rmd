...
The database does not contain an unequivocal information about the start and the end of a session, therefore we had to engineer this from the existing data. This was carried out in SQL (the dataset consists of ~4 million rows). In brief:

- The session_start event was chosen to define the start of the session. Within the same day, different event with this `event_name="session_start"` were assigned a rank according to their timestamp
- Every other event before the next session_start was labelled with the same rank
...
prop.test(x = prop_Thu$n_converted, n = prop_Thu$total, alternative = "less")
...
prop.test(x = prop_Fri$n_converted, n = prop_Fri$total, alternative = "less")
...
## Session length does not differ {.tabset}

Sessions during campaign sessions are slightly longer each day of the week, although the median value is associated with high variability.

What is interesting is that on Saturday the _purchasing_ campaign sessions are on average much shorter than the sessions not related to a campaign. However, a closer inspection shows that these are just 4 purchasing events, which makes a statistical comparison meaningless.



### TTP during or outside a campaign
...
There are also differences in the median amount order between campaign and no-campaign sessions in some days, however none of them is statistically significant.
...
```{r ROI, message=F, eval=F, include=F}

# Here we calculate the ROI, but we already did it in the Hands-on, so it is not
# reported again
...
cost_of_campaigns <- sum(marketing_cost$Cost)

ROI <- campaigns_revenues / cost_of_campaigns

ROI_pct <- (round(ROI,2) - 1) * 100
...
## TTP and revenues per day on- and off-campaign 
